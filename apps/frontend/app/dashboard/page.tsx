...
function App() {
  ...
  const processedWebsites = useMemo(() => {
    // Guard clause to handle undefined or null websites
    if (!websites || !Array.isArray(websites)) {
      return [];
    }

    return websites.map(website => {
      // Guard clause to handle websites without ticks
      if (!website.ticks || !Array.isArray(website.ticks)) {
        return {
          id: website.id,
          url: website.url,
          status: "unknown" as UptimeStatus,
          uptimePercentage: 0,
          lastChecked: 'Never',
          uptimeTicks: Array(10).fill("unknown") as UptimeStatus[],
        };
      }
        ...
        <div className="space-y-4">
          {processedWebsites.length === 0 ? (
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">No websites monitored yet</h3>
              <p className="text-gray-500 dark:text-gray-400 mb-4">Add your first website to start monitoring its uptime</p>
              <button
                onClick={() => setIsModalOpen(true)}
